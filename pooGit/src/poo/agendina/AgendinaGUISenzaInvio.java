...
class FinestraGUISenzaInvio extends JFrame {
	...
	private class FrameAL extends JFrame implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == capacita) {
				FinestraGUISenzaInvio.this.capacita = Integer.parseInt(capacita.getText());
			} else if (e.getSource() == ok) {
				agendina = new AgendinaAL(FinestraGUISenzaInvio.this.capacita);
				menuAvviato();
				this.dispose();
			}
		} // actionPerformed
	...
	private class FrameVector extends JFrame implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == capacita) {
				...
				agendina = new AgendinaVector(FinestraGUISenzaInvio.this.capacita);
				menuAvviato();
				this.dispose();
			}
		} // actionPerformed
	...
	private class FrameAggiungiNominativo extends JFrame implements ActionListener {
		...
		private boolean verificaUscita() {
			return !cognome.getText().equals("") && !nome.getText().equals("") &&
				!prefisso.getText().equals("") && !telefono.getText().equals("");
		} // verificaUscita
		private void clear() {
			cognome.setText("");
			nome.setText("");
	...
	private class FrameRimuoviNominativo extends JFrame implements ActionListener {
		...
		private void clear() { cognome.setText(""); nome.setText(""); }
	...
	private class FramePersonaDi extends JFrame implements ActionListener {
		...
		private void cercaPersona() {
			Nominativo n = agendina.cerca(prefisso.getText(), telefono.getText());
			this.setVisible(false);
			if (n != null)
		...
		private boolean verificaUscita() {
			return !prefisso.getText().equals("") && !telefono.getText().equals("");
		} // verificaUscita
		private void clear() {
			prefisso.setText("");
			telefono.setText("");
	...
	private class AscoltatoreEventiAzione implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == esci) {
				...
				impl = "AL";
				FinestraGUISenzaInvio.this.setTitle(titolo + impl);
				...
				try {
					...
					if (chooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
						...
						FinestraGUISenzaInvio.this.setTitle(titolo + impl + " - " + fileDiSalvataggio.getName());
					}
					if (fileDiSalvataggio != null)
						agendina.salva(fileDiSalvataggio.getAbsolutePath());
					else JOptionPane.showMessageDialog(null, "Nessun Salvataggio!");
...
} // FinestraGUISenzaInvio

public class AgendinaGUISenzaInvio {
	public static void main(String[]args) {
		FinestraGUISenzaInvio f = new FinestraGUISenzaInvio();
		f.setVisible(true);
	} // main
} // AgendinaGUISenzaInvio
