package it.unical.sisop.barMod;
...
public class BarLC extends Bar {
	...
	public void finisci(int i) throws InterruptedException {
		lock.lock();
		try {
			numPostiLiberi[i]++; //incremento il numero di posti liberi della i-esima fila

			if (fila[i].size() > 0) {
				occupato[i].signalAll();
			}

			System.out.format("Persona[%s] è uscita %s%n", Thread
