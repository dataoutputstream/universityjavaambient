...
public class Grafica extends JFrame implements ActionListener{
	...
	public Grafica() {
		...
		panel.setBounds(-100, 0, 820, 40);
		pannello.add(panel);
		
		JLabel espressione = new JLabel("Espressione");
		espressione.setForeground(Color.BLACK.darker().darker());
		...
		risultato.setForeground(Color.BLACK.darker().darker());
		panel.add(risultato);
		...
		tastiera.setBounds(-1, 41, this.getWidth(), 220);
	...
	private void risolvi() {
		...
		if(ValutaEspressioneStack.benformata(Input.getText()));
